...
{
	inline bool operator()(int x,int y)//重载操作符
    {
    	return dist[x]>dist[y];//>表示越往下越大即小根堆，故<是大根堆 
		}	
};
inline void addline(int x,int y,int z)//前向星就是邻接表 
{
	v[cnt]=y;//v存后驱 
	w[cnt]=z;//w存权值 
    ...
    while(!Q.empty())
    {
    	...
    	if(!tag[x])//每个节点只访问一次
    	{
    		...
    		for(register int i=head[x];i!=-1;i=nxt[i])
    		{
    			...
    			if(dist[x]+w[i]<dist[y]) dist[y]=dist[x]+w[i];
    			Q.push(y);//类似于层序遍历 
    			//y入队后并没有被访问 
			}
		}
...
int main()
{
	...
	for(register int i=1;i<=maxm;i++) head[i]=-1;
	for(register int i=1;i<=maxn;i++) tag[i]=0;
	for(register int i=1;i<=m;i++)//链式前向星 
	{
		int x,y,z;
		x=read(),y=read(),z=read();
		addline(x,y,z);
	}
	for(register int j=1;j<=n;j++)dist[j]=inf;//重要赋值 
	dijkstra(s);
	for(register int i=1;i<=n;i++) write(dist[i]),putchar(' ');
	return 0;
}
