...
//当n为正数时，n-1是把n的二进制表示的最右边的1变为0，这一位后面的0变为1，进行与运算自然可以消去n最右边的1
//当n为负数(补码)时，n-1的运算可以直接看成与正数相同，所以也能使进行与运算自然可以消去n最右边的1
//负数-1时其绝对值变大，但是在计算机中使用补码存储，负数是取反加一操作，因为取反操作的存在，使得正负在这里正负数的二进制变化看起来是相同的
int NumberOf1(int n)
{
    int count=0;
    while(n)
